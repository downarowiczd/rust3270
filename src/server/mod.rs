        ...
        while !events.is_empty() || !extra_events.is_empty() {
            ...
            for mut event in events.drain(..) {
                match event {
                    ...
                    TelnetEvents::DataReceive(ref mut data) => self.cur_record.extend_from_slice(&data[..]),
                    ...
                    TelnetEvents::Subnegotiation(TelnetSubnegotiation { option: tn_opt::TTYPE, buffer }) => {
                        if buffer[0] == 0 {
                            ...
                            debug_msg!("Terminal type: {}", String::from_utf8_lossy(self.term_type.as_ref().unwrap()));
                        }
                    }
